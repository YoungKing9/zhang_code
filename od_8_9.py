...
def make_od_6_10():
    path = '/data/WorkMind/data/student/d/one_hour/8_9.txt'
    save_path = '/data/WorkMind/data/student/d/one_hour/od_910.txt'
                ...
                if car[0] == car2[0] and car2[3] != car[3]:
                    ...
                    for iii, h in enumerate(f[key + ii + 2:]):
                        ...
                        if int(key + ii + 2 + iii) >= len(f):
                            ...
                            break
                        if car[0] == car3[0] and car3[3] != car2[3]:
                            car_dic[int(car[0])] = int(key + ii + 2 + iii)
                            # and int(car3[6]) - int(car2[6]) > 600:  # 此时下车了，状态是空车，下面无论怎么执行都要break
